...
/* POST /api/client/stops/[stop_id]/cancel-request/deny — dispatcher denies
 * a pending cancellation request. Stop stays active, reason recorded. */
export async function POST(request: Request, { params }: { params: Promise<{ stop_id: string }> }) {
  ...
  const upstream = await fetch(
    `${FASTAPI_BASE}/v1/stops/${encodeURIComponent(stop_id)}/cancel-request/deny?tenant_id=${tenantId}`,
    {
      method: "POST",
