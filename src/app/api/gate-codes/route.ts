...
export async function GET(req: NextRequest) {
  try {
    ...
    const [list, count] = await Promise.all([
      db.collection("gate_codes").find(filter).sort({ created_at: -1 }).limit(100).toArray(),
      db.collection("gate_codes").countDocuments(filter),
    ]);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const db = await getDb();
    const result = await db.collection("gate_codes").deleteOne({
      _id: new ObjectId(id),
      tenant_id: 1,
    });
